...
- **改法已调整**：本报告的混入实现已换成集成层调用（`BaritoneThreadPool#daemonize()`，由模组初始化调用），原因与依据见 172 第二节。
- **本报告第五节「26.2 混入不生效」的观察不再作为结论**：那几轮 26.2 验收的实例里同时存在混淆包与个人版 jar，Fabric 只加载其中一个且日志不提示，无法确认跑的是哪个 jar。详见 172 第三节。
- **发布包已重出**：线上两个附件已覆盖为含本修复的产物（报告 172）。
