# 135-复盘-迁移到 Minecraft 26.2（双版本并行：26.1.2 留 master，26.2 走 port/26.2）
...
| 自绘界面 | ⚠️ 部分 | 日志观测到 `界面观察：ClickGuiScreen`，说明 Skija 界面链路活着；**像素级视觉正确性未核对** |
| 26.2 完整功能回归 | ⚠️ 未做 | 需用户实机跑一遍各模块 |
...
Fabric Loader 直接拒绝启动。验证时临时把该目录移开、跑完**已原样还原**。
`run/` 在 `.gitignore` 内，不影响仓库与产物。
...
1. **自绘 UI 的视觉核对**：请实机打开控制台界面确认排版、字牌、模糊、物品图标都正常
   （本次只到「不崩、界面被观测到」这一层）。
