...
<#
  237 · 真实用户环境（100+ Mods + Iris / Sodium + 光影）兼容 smoke 驱动器（26.2 线）。

  为什么要单独跑一次：236 改过渲染器（多矿物快照）、Worker / runtime 与维度语义，
  「开发者自己的 100+ Mods 环境还能不能正常起来」是一条独立证据（236 报告的限制第 7 条）。

  铁律（本脚本只读源目录、只写验收目录）：
    · 源：D:\mcaddon\yiyiaddon\run-26.2 —— 开发者的真实客户端，**只读**；
    · 目标：<仓库>\.dev-runs\26.2\seed\final-user-env —— 只写这里；
...
[CmdletBinding()]
param(
    [string]$SourceRunDir = 'D:\mcaddon\yiyiaddon\run-26.2',
    [int]$ClientTimeoutSeconds = 1800
)

$ErrorActionPreference = 'Stop'
$repoRoot = Split-Path -Parent $PSScriptRoot
$target = Join-Path $repoRoot '.dev-runs\26.2\seed\final-user-env'
...
# ModernFix 的开发环境专用混入与 26.2 不兼容：用该 Mod 自带的开关关掉它（只改验收目录副本，绝不碰 run-26.2）。
# 事实：modernfix 5.27.19-build.1 的 devenv 包对 MinecraftServer#createUserApiService 做 @Overwrite，
# 而 26.2 里该方法已不是 static ⇒ 加载期 InvalidMixinException，客户端在装置挂载前就崩。
# 这是「第三方 Mod 的 dev 专用混入」问题（正式客户端根本不加载 devenv 包），不是本模组的问题；
# 处置走 ModernFix 配置里第一段就写明的兼容开关（mixin.devenv=false），103 个 Mod 一个不少。
$modernFixConfig = Join-Path $target 'config\modernfix-mixins.properties'
$modernFixPatched = $false
if (Test-Path -LiteralPath $modernFixConfig) {
    $mfLines = @(Get-Content -LiteralPath $modernFixConfig -Encoding UTF8)
    if (-not ($mfLines | Where-Object { $_ -match '^\s*mixin\.devenv\s*=' })) {
        $mfLines += 'mixin.devenv=false'
        Set-Content -LiteralPath $modernFixConfig -Encoding UTF8 -Value $mfLines
        $modernFixPatched = $true
        Write-Step 'ModernFix：已按该 Mod 自带开关关闭 devenv 混入（26.2 下它与 MinecraftServer 签名不兼容）'
    }
}

# 必要 options：真实用户的 options.txt（渲染距离 / 光影开关 / 视角等），只补三个「自动装置必须的」开关。
# pauseOnLostFocus 必须关：自动化运行拿不到窗口焦点，原版会在失焦时打开暂停菜单，单人世界随之暂停
# （26.2 实测：暂停后装置停在用例 43/102，随后整局退出、报告没落盘）。它只影响本验收目录的副本。
...
$wanted = @{ 'onboardAccessibility' = 'false'; 'skipMultiplayerWarning' = 'true'; 'pauseOnLostFocus' = 'false' }
$seen = @{}
...
$judgementLines = @(
    '《237 · 真实用户环境（100+ Mods + Iris / Sodium）兼容 smoke 判决（26.2 线）》',
    "源目录（只读）：$SourceRunDir",
    ...
    "ModernFix devenv 混入：按该 Mod 自带开关关闭（$modernFixPatched；26.2 下该 dev 专用混入与 MinecraftServer 签名不兼容）",
