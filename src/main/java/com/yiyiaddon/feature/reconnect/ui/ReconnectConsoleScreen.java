...
import com.yiyiaddon.feature.reconnect.ui.console.ReconnectOverviewPage;
import com.yiyiaddon.feature.reconnect.ui.console.ReconnectSettingsPage;
...
/**
 * 自动重连控制台：把 6 项设置与运行状态拆成两页（概览 / 重连设置）。
 *
 * <p><b>骨架逐字照既有控制台</b>（自动骨粉 / 自动登入 / 发包秒破：同源件
 * {@link com.yiyiaddon.ui.console.ConsoleWidgets} 直接复用）：窗口标题、模块说明副标题、
 * 顶栏 {@link ConsoleHeaderBar}（状态文字 + 快捷键徽章 + 模块开关）、状态条（两行六格）、页签行、
 * 可整体替换的 {@link Body}、{@link #reload()} 重建、{@link #switchTab} 延到下一 tick、
 * 每秒自动刷新（只在概览页整页重建）、底部「刷新 / 关闭」、tooltip 悬浮层 —— 全部同一套写法与同一套数值。</p>
 *
 * <p><b>页签 = 分组</b>：概览页只读运行状态与三个动作（立即重连 / 停止重连 / 测试重连），
 * 设置页只放设置项，页签名就是设置组名（第 183 条）。</p>
 */
public final class ReconnectConsoleScreen extends PanelScreen implements ConsoleHost {

    /** 自动刷新的 tick 订阅所有者标识（本窗口独占） */
    private static final String TICK_OWNER = "screen.reconnect.console";
    ...
    /** 状态条每秒重排时的第二行格子数（与其它控制台一致：两行三列） */
    private static final int STATUS_COLUMNS = 3;
    ...
    /** 页签：一屏只显示一类内容（概览 / 设置组） */
    private enum Tab {
        OVERVIEW(ReconnectTexts.TAB_OVERVIEW),
        SETTINGS(ReconnectTexts.GROUP_SETTINGS);
     ...
     */
    public ReconnectConsoleScreen(Screen parent, AutoReconnectModule module) {
        super(ReconnectTexts.MODULE_NAME + "控制台", parent);
        this.module = module;
     ...
     * 每秒取一次新快照。
     *
     * <p><b>概览页：</b>整页重画（纯读数 + 三个动作按钮，无输入控件）。<b>设置页：</b>只换状态条，
     * 不重建正文 —— 正文带可交互控件，每秒重建会把正在编辑的数值框与正在点的开关一起抹掉。</p>
     */
    private void onTick() {
    ...
    public void reload() {
        // 整页重建会丢弃全部控件实例，先把输入焦点交还，避免焦点留在已消失的控件上
    ...
    private void buildInto(CompactStack stack) {
        ...
        switch (tab) {
            case OVERVIEW -> new ReconnectOverviewPage(this, module).build(stack);
            case SETTINGS -> new ReconnectSettingsPage(this, module).build(stack);
        }
    ...
    private void buildFooter(CompactStack stack) {
        stack.add(new ButtonStrip(this, List.of(
            new Ctl(new Button(ReconnectTexts.BTN_REFRESH, this::reload),
    ...
    private static final class Status {
        ...
        /** 取值全部来自模块只读接口与引擎只读接口，本类不改任何状态 */
        private static Status capture(AutoReconnectModule module) {
            if (module == null) return empty();
            ReconnectSettings settings = module.settings();

            return new Status(new String[]{
                "§7模块：" + (module.isEnabled() ? "§a运行中" : "§8未启用"),
                "§7状态：" + module.stateText(),
                "§7重连次数：" + module.attemptsText(),
                "§7服务器：" + (module.engine().hasServer()
                    ? "§f" + module.engine().serverAddressText() : "§8无记录"),
                "§7接管方：" + module.ownerText(),
                "§7等待 / 稳定：" + seconds(settings.delayTicks) + " §8/ §f" + seconds(settings.stableTicks)
            });
        }
    ...
    private final class StatusStrip implements CompactElement {
        ...
        public void draw(Canvas canvas, float x, float y, float width, float alpha,
            ...
            for (int i = 0; i < STATUS_COLUMNS * 2; i++) {
                int row = i / STATUS_COLUMNS;
                int column = i % STATUS_COLUMNS;
                float cellX = x + CELL_PAD + Math.max(0f, width - CELL_PAD * 2f) * column / STATUS_COLUMNS;
                drawCell(canvas, snapshot.cell(i), cellX, y + ROW_HEIGHT * row,
                    Math.max(0f, width - CELL_PAD * 2f) / STATUS_COLUMNS, alpha);
            }
        }
