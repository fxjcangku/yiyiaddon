...
import com.yiyiaddon.feature.admindetect.ui.console.AdminDetectPage;
import com.yiyiaddon.feature.admindetect.ui.console.AdminListPage;
import com.yiyiaddon.feature.admindetect.ui.console.AdminOverviewPage;
import com.yiyiaddon.feature.admindetect.ui.console.AdminRenderPage;
...
/**
 * 管理员检测控制台：把模块的设置与状态按用途拆成页签。
 *
 * <p><b>骨架逐字照杀戮光环 / 自动挖矿 / 星露谷控制台</b>（2026-09-16 抽出的通用件
 ...
 *
 * <p><b>设置项一字未改</b>：四页里的每一行都来自 {@link AdminDetectorSettings} 的同名字段，
 * 名称与描述取自 {@link com.yiyiaddon.feature.admindetect.config.AdminDetectorTexts}；
 * 控制台只负责「搬到哪一页、按什么顺序排」，不新增任何设置项、不改默认值与取值域。
 * 旧「自动断线」模块合并进来后不再有独立页面，它的 {@code §c立即断线} 按钮落在「概览」页。</p>
 */
public final class AdminDetectorConsoleScreen extends PanelScreen implements ConsoleHost {

    /** 自动刷新的 tick 订阅所有者标识（本窗口独占） */
    private static final String TICK_OWNER = "screen.admindetect.console";
    ...
    private void buildInto(CompactStack stack) {
        ...
        switch (tab) {
            case OVERVIEW -> new AdminOverviewPage(this, module).build(stack);
            case DETECT -> new AdminDetectPage(this, module).build(stack);
            case LIST -> new AdminListPage(this, module).build(stack);
            case RENDER -> new AdminRenderPage(this, module).build(stack);
        }
    ...
    private static final class Status {
        ...
        private static Status capture(AdminDetectorModule module) {
            if (module == null) return empty();
            AdminDetectorSettings settings = module.settings();
            return new Status(new String[]{
                "§7模块：" + (module.isEnabled() ? "§a运行中" : "§8未启用"),
                "§7范围内威胁：§f" + module.threats().size(),
                "§7最近威胁：§f" + nearestThreat(module),
                "§7检测范围：§f" + settings.detectRange + "§7 格",
                "§7名单：§f白 " + settings.whitelist.size() + " §8/ §f黑 " + settings.blacklist.size(),
                module.isDisconnecting() ? "§7断线：§c已触发" : "§7断线：§a就绪"
            });
        }

        /** 范围内第一个威胁的名字（模块按世界实体顺序排列）；没有则 {@code §8无} */
        private static String nearestThreat(AdminDetectorModule module) {
            for (Entity threat : module.threats()) {
                if (threat != null) return threat.getName().getString();
            }
            return "§8无";
        }
