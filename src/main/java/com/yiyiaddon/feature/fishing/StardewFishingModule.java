 ...
 * 中文注释写清楚开发中，展示无法使用红色字」。本类只做三件事，让这个「还没做的模块」在界面上不骗人：</p>
 *
 * <ol>
 *     <li>{@code description()} 写清「开发中」——模块行注释与模块页副标题共用它，
 *         长度按「一行放得下的中文短注」控制（166 号复盘第十一节）；</li>
 *     <li>{@link #statusBadgeText()} 把模块行右侧的红色状态标记换成「无法使用」——
 *         默认文案是「未启用」，会让玩家以为点一下就能开；</li>
 ...
 *
 * <p>实现时把上面三条「占位说明」一并撤掉：补 {@code page()}、删 {@code statusBadgeText()}
 * 与 {@code environmentRefusal()} 覆写、按实际玩法重写 {@code description()}。</p>
 */
public final class StardewFishingModule extends Module {
    ...
    /** 行注释（模块行 / 模块页副标题共用）：开发中 + 无法使用，一眼看清这模块还不能用。 */
    private static final String DESCRIPTION = "开发中，暂时无法使用";

    /** 模块行状态标记的唯一取值（红色档）。 */
    private static final String STATUS_BADGE = "无法使用";
    ...
    /** 行状态标记覆写：红色「无法使用」，替掉会让人误以为能开的「未启用」。 */
    @Override
