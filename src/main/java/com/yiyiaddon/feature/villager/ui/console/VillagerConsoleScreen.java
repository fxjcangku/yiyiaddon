...
/**
 * 自动村民交易控制台：把旧模块页承载不下的设置项按用途拆成整屏页签（D5 拍板）。
 *
 * <p><b>骨架逐字照自动农场控制台</b>（通用件 {@link com.yiyiaddon.ui.console.ConsoleWidgets}
 * 与 {@link ConsoleHost} 直接复用）：窗口标题、状态条（两行八格）、页签行、可整体重建的正文容器、
 * 每秒自动刷新、底部「刷新 / 关闭」、tooltip 走全局 {@link TooltipLayer}——同一套写法与同一套数值。</p>
 *
 * <p><b>页签划分（D5）：</b>概览（只读实况）/ 设置（默认组 7 项平铺 + 按职业折叠分组 /
 * 多任务职业勾选）/ 点位（两张点位卡，与模块页同一份 {@code VillagerPointCards}）/
 * 日志（状态播报流水）。页签名与顺序逐字固定，不得改名或增删。</p>
 *
 * <p><b>键位录制（D4）：</b>设置页「快速停止键」行用自研键位控件
 * {@link com.yiyiaddon.ui.widget.SettingKeybind}（照旧框架 {@code WKeybind} 自研，见其类 javadoc）。
 * 本窗口负责把按键与鼠标点击转发给录制（{@link com.yiyiaddon.ui.widget.SettingKeybind#keyPressed} /
 * {@code #mousePressed}，与 {@code ModuleScreen#keyPressed} 同一写法）——不转发的话，
 * 录制态永远等不到输入。</p>
 *
 * <p><b>折叠记忆：</b>折叠状态存本窗口实例（{@link #collapsedSections()}）。整页重建
 * （刷新 / 切页签 / 改模式）会丢弃全部控件实例，状态存窗口侧才不会每次回到默认值。
 * 设置页的折叠分组是按职业动态生成的，键无法在构造期一次列全，因此由页面在装配时调
 * {@link #ensureCollapsed(String)} 登记默认收起（见该方法的「为什么」）。</p>
 */
public final class VillagerConsoleScreen extends PanelScreen implements ConsoleHost {

    /** 自动刷新的 tick 订阅所有者标识（本窗口独占） */
    private static final String TICK_OWNER = "screen.villager.console";

    /** 自动刷新间隔：20 tick 一次（自动农场 / 挖矿 / 星露谷控制台同口径） */
    private static final int AUTO_REFRESH_TICKS = 20;

    /** 窗口标题：模块名 + 控制台（与自动农场 / 挖矿控制台同一命名口径） */
    private static final String WINDOW_TITLE = AutoVillagerTradeModule.MESSAGE_MODULE + "控制台";

    // 排版常量（与其它控制台共用同一套数值：行内构件的尺寸取自 ConsoleMetrics）
    private static final float LINE_GAP = 6f;
    private static final float TEXT_SIZE = 11f;

    /** 页签：一屏只显示一类内容（D5 固定四页）。 */
    private enum Tab {
        OVERVIEW("概览"),
        SETTINGS("设置"),
        POINTS("点位"),
    ...
    /** 已经按默认值登记过的折叠键（动态分组首次出现时登记一次「默认收起」，之后用户的展开不被回填） */
    private final Set<String> knownSections = new HashSet<>();

    /** 模块页入口（{@code AutoVillagerTradePage#openConsole}）使用的一参构造：上级窗口取当前屏幕 */
    public VillagerConsoleScreen(AutoVillagerTradeModule module) {
        this(currentScreen(), module);
    }

    /**
     * @param parent 上级屏幕（模块页）——ESC / 返回键回到它
     * @param module 归属模块：控制台读写它的设置、绑定与日志，不新增任何模块侧成员
     */
     ...
     * 键盘转发：录制态优先。
     *
     * <p><b>为什么必须在这里转发</b>：{@link SettingKeybind} 的录制态是控件侧的静态槽，
     * 真正的按键要靠界面转交 {@link SettingKeybind#keyPressed(net.minecraft.client.input.KeyEvent)}
     * （与 {@code PanelScreen} 转交输入框同一手法）。本骨架只转发输入框，因此这里补齐
     * （与 {@code ModuleScreen#keyPressed} 逐字同序：先录制、再输入框、最后父类）。</p>
     */
    @Override
    ...
    /**
     * 鼠标转发：录制态优先（键位可以绑鼠标按键，照旧框架 {@code WKeybind#onAction} 允许鼠标绑定）。
     *
     * <p>录制中吞掉整次点击：否则点一下就想绑的侧键会同时触发下方控件。</p>
     */
    @Override
    ...
    /** 每秒取一次新快照：概览与点位页整页重画（纯读数），其余页只换状态条不重建正文（保护输入框） */
    private void onTick() {
     ...
     * <p><b>为什么</b>：改「运行模式 / 目标职业 / 多任务职业勾选」会整组换掉设置页的折叠分组，
     * 必须重建页面；但当场重建会把正在响应的那个控件（模式循环块 / 勾选开关）连同控件树一起摘掉，
     * 抬起事件落到空处，实机表现为「点了没反应」。与 {@link #switchTab} 同一手法
     * （本项目控制台既定做法，见 {@code AutoFarmConsoleScreen#switchTab}）。</p>
     */
     ...
     * 动态折叠分组的「默认收起」登记。
     *
     * <p><b>做什么</b>：首次见到某个折叠键时把它登记为收起；已经登记过的键不再动。</p>
     * <p><b>为什么需要它</b>：{@code FoldSection} 的折叠语义是「键在集合里 = 收起」，
     * 而设置页的分组是按职业动态生成的（键 = {@code profession:<职业名>} 与 {@code pipeline}），
     * 构造期无法一次列全，不登记就会全部默认展开（旧项目该三组是分组标题，本项目按第 195/196 条默认收起）。
     * 用一张「已登记」表而不是每次重建都 add，是为了让玩家手动展开后不被回填。</p>
     */
    public void ensureCollapsed(String key) {
        if (key == null) return;
        if (knownSections.add(key)) collapsedSections.add(key);
    }
    ...
    private void buildInto(CompactStack stack) {
        ...
        switch (tab) {
            case OVERVIEW -> new VillagerOverviewPage(this, module).build(stack);
            case SETTINGS -> new VillagerSettingsPage(this, module).build(stack);
            case POINTS -> new VillagerPointPage(this, module).build(stack);
            case LOGS -> new VillagerLogPage(this, module).build(stack);
        }
    ...
    /**
     * 状态条两行八格的只读快照
     * （格位：模块 / 状态机 / 模式 / 职业 / 物品 / 绿宝石箱 / 成品交易箱 / 自检）。
     */
    private static final class Status {
         ...
         * 自检格与模块启用前的自检同一份判据（{@link AutoVillagerTradeModule#selfCheck()}），
         * 缺项数一眼可见，避免进了控制台还要去开模块才知道缺什么。</p>
         */
        private static Status capture(AutoVillagerTradeModule module) {
            ...
            return new Status(new String[]{
                ...
                "§7状态机：§f" + module.fsm().getCurrentState().cn(),
                "§7模式：§f" + settings.mode,
                "§7职业：§f" + professionCell(module),
                "§7物品：§f" + itemCell(module),
                "§7绿宝石箱：" + boundCell(VillagerBindingStore.getEmeraldChestPos() != null),
                "§7成品交易箱：" + boundCell(VillagerBindingStore.getUnloadChestPos() != null),
                missing.isEmpty() ? "§a自检通过" : "§e自检缺 " + missing.size() + " 项"
            });
        }

        /**
         * 「职业」格：多任务模式列已勾选职业（未勾选灰字），其余模式给当前目标职业。
         *
         * <p>名单取设置页的 {@link VillagerSettingsPage#visibleProfessions(AutoVillagerTradeModule)}
         * （等价旧 {@code isProfessionVisible :294-301}），与设置页铺分组同一判据，不另判一次。</p>
         */
        private static String professionCell(AutoVillagerTradeModule module) {
            List<String> visible = VillagerSettingsPage.visibleProfessions(module);
            return visible.isEmpty() ? "§8未勾选" : String.join("、", visible);
        }

        /** 「物品」格：当前可见职业已选物品条数（未选择灰字） */
        private static String itemCell(AutoVillagerTradeModule module) {
            VillagerTradeSettings settings = module.settings();
            int count = 0;
            for (String name : VillagerSettingsPage.visibleProfessions(module)) {
                count += settings.itemTargets(name).size();
            }
            return count == 0 ? "§8未选择" : count + " 项";
        }

        /** 绑定格：已绑定绿字、未绑定灰字（旧自检 {@code §a绿宝石箱§f·未绑定} 的正反两面） */
        private static String boundCell(boolean bound) {
            return bound ? "§a已绑定" : "§8未绑定";
        }
