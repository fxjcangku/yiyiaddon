...
public final class MiningSettings {
    ...
    // ━━━ 种子目标模式（235 接入 / 238 起支持全部 11 种矿物；旧项目有对应物，本项目本轮接上） ━━━

    /**
     * 种子目标模式｜用<b>已通过验证的种子预测</b>作为挖矿目标（精确到方块），
     * 而不是让 Baritone 按「某种矿石」这个类型在附近全局搜。
     *
     * <p><b>追哪种矿</b>：由「种子挖矿」页勾选决定 —— 一次只追一种（与自动挖矿设置口径一致），
     * 多选时按维度声明序取第一种；该矿的深层变种（例如深层钻石矿）算同一种，不需要分别勾选。
     * 主世界 8 种（钻石 / 红石 / 青金石 / 金 / 铁 / 铜 / 煤 / 绿宝石）与下界 3 种
     * （远古残骸 / 下界石英 / 下界金）都可以追。</p>
     *
     ...
     *
     * <p>前置条件（自检逐条报出）：种子挖矿总开关已开 + 服务器种子已填写且格式合法 +
     * 当前维度受支持（主世界 / 下界）+ 种子验证「已验证」且<b>证据覆盖要追的那种矿</b> +
     * 自动挖矿的目标矿物与该矿一致（时运 / 精准均可）+ 秒破已开。
     * 下界另需<b>下界专属验证</b>建立（主世界已验证不等于下界已验证）。</p>
     */
