...
public final class AutoMinerPage extends CompactModulePage implements ModulePage {
    ...
    private static final HelpPanelScreen.HelpSection[] HELP_SECTIONS = {
        ...
        new HelpPanelScreen.HelpSection("自用模式 §7(挖够就自己去卖)",
            ...
            "  §b▸ §f卖不动会点名 §8— §7出售时连续几轮一颗未少就停机播报，并点名「点商品 → 点「出售数量关键词」"
                + "→ 点「确认出售关键词」」这三步在哪一环没生效；那一刻的整屏菜单会逐槽写进 "
                + "logs/latest.log 的 §f[卖矿流程]§7，照着槽位名把关键词填对即可",
            ...
            "  §e⚠ §f「触发条件」页在自用模式下少两行 §8— §7与卸货绑定的「满载组数」「潜影盒打包机」不显示"
                + "（自用模式不走卸货：挖够组数直接去卖，阈值在「自用模式」页的「触发组数」）"
        ),
