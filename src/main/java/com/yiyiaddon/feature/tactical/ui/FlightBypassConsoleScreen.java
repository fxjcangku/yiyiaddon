...
import com.yiyiaddon.feature.tactical.ui.console.FlightBypassOverviewPage;
import com.yiyiaddon.feature.tactical.ui.console.FlightBypassSettingsPage;
...
/**
 * 飞行绕过控制台：把 6 项设置按旧项目原 2 个设置组 + 概览拆成页签
 * （概览 / 模式选择 / 参数调整）。
 *
 * <p><b>骨架逐字照传送控制台</b>（同源件 {@link com.yiyiaddon.ui.console.ConsoleWidgets} 直接复用）：
 ...
 *
 * <p><b>设置项一字未改</b>：每行都来自 {@link FlightBypassSettings} 的同名字段，名称、默认值与取值域
 * 逐字取自旧项目 {@code tactical/FlightBypass.java} 的设置声明；控制台只负责「搬到哪一页、按什么顺序排」，
 * 不新增设置项、不做「手动触发」这类旧项目没有的功能按钮（旧项目只有模式与参数，没有面板动作）。</p>
 */
public final class FlightBypassConsoleScreen extends PanelScreen implements ConsoleHost {

    /** 自动刷新的 tick 订阅所有者标识（本窗口独占） */
    private static final String TICK_OWNER = "screen.flightbypass.console";
     ...
     */
    public FlightBypassConsoleScreen(Screen parent, FlightBypassModule module) {
        super("飞行绕过控制台", parent);
        this.module = module;
    ...
    private void buildInto(CompactStack stack) {
        ...
        switch (tab) {
            case OVERVIEW -> new FlightBypassOverviewPage(this, module).build(stack);
            case MODE -> new FlightBypassSettingsPage(this, module).buildMode(stack);
            case PARAMS -> new FlightBypassSettingsPage(this, module).buildParams(stack);
        }
    ...
    private static final class Status {
        ...
        /** 取值全部来自模块设置与协调器只读接口，本类不改任何状态 */
        private static Status capture(FlightBypassModule module) {
            if (module == null) return empty();
            FlightBypassSettings settings = module.settings();

            return new Status(new String[]{
                "§7模块：" + (module.isEnabled() ? "§a运行中" : "§8未启用"),
                "§7当前模式：§f" + settings.mode.displayName,
