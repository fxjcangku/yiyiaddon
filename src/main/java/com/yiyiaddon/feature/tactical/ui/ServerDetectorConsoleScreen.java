...
import com.yiyiaddon.feature.tactical.ui.console.ServerDetectorOverviewPage;
import com.yiyiaddon.feature.tactical.ui.console.ServerDetectorSettingsPage;
...
/**
 * 服务器检测控制台：把 8 项设置按旧项目原 2 个设置组 + 概览拆成页签
 * （概览 / 底裤侦测 / 资源包劫持）。
 *
 * <p><b>骨架逐字照飞行绕过控制台</b>（同源件 {@link com.yiyiaddon.ui.console.ConsoleWidgets} 直接复用）：
 * 窗口标题、模块说明副标题、顶栏 {@link ConsoleHeaderBar}（模块开关 + 快捷键徽章）、状态条（两行六格）、
 * 页签行、可整体替换的 {@link Body}、{@link #reload()} 重建、{@link #switchTab} 延到下一 tick、
 * 每秒自动刷新（只在概览页整页重建）、底部「刷新 / 关闭」、tooltip 悬浮层 —— 全部同一套写法与同一套数值。</p>
 *
 * <p><b>设置项一字未改</b>：每行都来自 {@link ServerDetectorSettings} 的同名字段，名称、描述、默认值
 * 与取值域逐字取自旧项目 {@code tactical/ServerDetector.java} 的设置声明；控制台只负责「搬到哪一页、
 * 按什么顺序排」，不新增设置项。</p>
 */
public final class ServerDetectorConsoleScreen extends PanelScreen implements ConsoleHost {

    /** 自动刷新的 tick 订阅所有者标识（本窗口独占） */
    private static final String TICK_OWNER = "screen.serverdetect.console";
    ...
    /** 页签：一屏只显示一类内容（页签名即旧项目 2 个设置组名 + 概览）。 */
    private enum Tab {
        OVERVIEW("概览"),
        DETECTION("底裤侦测"),
        RESOURCE_PACK("资源包劫持");
     ...
     */
    public ServerDetectorConsoleScreen(Screen parent, ServerDetectorModule module) {
        super("服务器检测控制台", parent);
        this.module = module;
    ...
    private void buildInto(CompactStack stack) {
        ...
        switch (tab) {
            case OVERVIEW -> new ServerDetectorOverviewPage(this, module).build(stack);
            case DETECTION -> new ServerDetectorSettingsPage(this, module).buildDetection(stack);
            case RESOURCE_PACK -> new ServerDetectorSettingsPage(this, module).buildResourcePack(stack);
        }
    ...
    private static final class Status {
        ...
        /** 取值全部来自协调器只读接口，本类不改任何状态 */
        private static Status capture(ServerDetectorModule module) {
            ...
            return new Status(new String[]{
                ...
                "§7服务器核心：" + coreText(),
                "§7反作弊：" + antiCheatText(),
                "§7累计拉回：§f" + TacticalCoordinator.getRubberBandTotal() + " §7次",
                "§7拉回冷却：" + (TacticalCoordinator.isRubberBandCooldown() ? "§e冷却中 2 秒" : "§8无"),
        ...
        private static String coreText() {
            ...
            if (name == null || name.isBlank() || "未知".equals(name) || "未检测".equals(name)) {
                return "§8" + (name == null || name.isBlank() ? "未知" : name);
            }
            return "§f" + name;
        }

        /** 反作弊显示名：未检测 / 未发现 / 未知时压成灰字，避免看起来像已命中 */
