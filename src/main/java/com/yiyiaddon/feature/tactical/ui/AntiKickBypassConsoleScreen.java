...
import com.yiyiaddon.feature.tactical.ui.console.AntiKickBypassOverviewPage;
import com.yiyiaddon.feature.tactical.ui.console.AntiKickBypassSettingsPage;
...
/**
 * 发包防踢控制台：把 18 项设置按旧项目原 6 个设置组 + 概览拆成页签
 * （概览 / 伪装客户端 / 聊天排队 / 防挂机 / 限制发包 / 拉回分析 / 模拟真人）。
 *
 * <p><b>骨架逐字照飞行绕过 / 服务器检测控制台</b>（同源件
 ...
 *
 * <p><b>设置项一字未改</b>：每行都来自 {@link AntiKickBypassSettings} 的同名字段，名称、描述、默认值
 * 与取值域逐字取自旧项目 {@code tactical/AntiKickBypass.java} 的设置声明；控制台只负责
 * 「搬到哪一页、按什么顺序排」，不新增设置项、不做「手动触发」这类旧项目没有的功能按钮。</p>
 */
public final class AntiKickBypassConsoleScreen extends PanelScreen implements ConsoleHost {

    /** 自动刷新的 tick 订阅所有者标识（本窗口独占） */
    private static final String TICK_OWNER = "screen.antikick.console";
    ...
    /** 页签：一屏只显示一类内容（页签名即旧项目 6 个设置组名 + 概览）。 */
    private enum Tab {
        OVERVIEW("概览"),
        DISGUISE("伪装客户端"),
        CHAT("聊天排队"),
     ...
     */
    public AntiKickBypassConsoleScreen(Screen parent, AntiKickBypassModule module) {
        super("发包防踢控制台", parent);
        this.module = module;
    ...
    private void buildInto(CompactStack stack) {
        ...
        switch (tab) {
            case OVERVIEW -> new AntiKickBypassOverviewPage(this, module).build(stack);
            case DISGUISE -> new AntiKickBypassSettingsPage(this, module).buildDisguise(stack);
            case CHAT -> new AntiKickBypassSettingsPage(this, module).buildChatQueue(stack);
            case ANTI_AFK -> new AntiKickBypassSettingsPage(this, module).buildAntiAfk(stack);
            case THROTTLE -> new AntiKickBypassSettingsPage(this, module).buildThrottle(stack);
            case ANALYSIS -> new AntiKickBypassSettingsPage(this, module).buildAnalysis(stack);
            case HUMAN -> new AntiKickBypassSettingsPage(this, module).buildHuman(stack);
        }
    ...
    private static final class Status {
        ...
        /** 动态限速倍率：被打折时黄字，正常为白字 */
        private static String throttleText(AntiKickBypassModule module) {
