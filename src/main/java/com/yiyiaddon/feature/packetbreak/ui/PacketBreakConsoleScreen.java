...
import com.yiyiaddon.feature.packetbreak.ui.console.PacketBreakOverviewPage;
import com.yiyiaddon.feature.packetbreak.ui.console.PacketBreakSettingsPage;
...
/**
 * 发包秒破控制台：把 21 项设置按旧项目原 4 个设置组 + 概览拆成页签
 * （概览 / 目标选择 / 发包参数 / 防同步与反作弊 / 进度显示）。
 *
 * <p><b>骨架逐字照飞行绕过 / 发包防踢控制台</b>（同源件
 * {@link com.yiyiaddon.ui.console.ConsoleWidgets} 直接复用）：窗口标题、模块说明副标题、
 * 顶栏 {@link ConsoleHeaderBar}（模块开关 + 快捷键徽章）、状态条（两行六格）、页签行、
 * 可整体替换的 {@link Body}、{@link #reload()} 重建、{@link #switchTab} 延到下一 tick、
 * 每秒自动刷新（只在概览页整页重建）、底部「刷新 / 关闭」、tooltip 悬浮层
 * —— 全部同一套写法与同一套数值，不自造第二套分页壳（第 181 条）。</p>
 *
 * <p><b>设置项一字未改</b>：每行都来自 {@link com.yiyiaddon.feature.packetbreak.config.PacketBreakSettings}
 * 的同名字段，名称、描述、默认值与取值域逐字取自旧项目
 * {@code tactical/packetbreak/PacketInstantBreak.java:111-272} 的设置声明；控制台只负责
 * 「搬到哪一页、按什么顺序排」，不新增设置项、不做旧项目没有的功能按钮。</p>
 */
public final class PacketBreakConsoleScreen extends PanelScreen implements ConsoleHost {

    /** 自动刷新的 tick 订阅所有者标识（本窗口独占） */
    private static final String TICK_OWNER = "screen.packetbreak.console";
    ...
    /** tooltip 文字基准色：深色主题白字（主题色槽的深色版本即纯白，此处具名以免散落字面量） */
    private static final int TIP_COLOR_DARK = 0xFFFFFF;

    /** tooltip 文字基准色：深色主题白字；浅色主题用主文字色 */
    private static int tipColor(ClickGuiThemeColors tc) {
        return tc != null && !tc.dark ? tc.primaryText : TIP_COLOR_DARK;
    }

    /** 页签：一屏只显示一类内容（页签名即旧项目 4 个设置组名 + 概览）。 */
    private enum Tab {
        OVERVIEW(PacketBreakTexts.TAB_OVERVIEW),
        TARGET(PacketBreakTexts.GROUP_TARGET),
        PACKET(PacketBreakTexts.GROUP_PACKET),
        ANTICHEAT(PacketBreakTexts.GROUP_ANTICHEAT),
        RENDER(PacketBreakTexts.GROUP_RENDER);
     ...
     */
    public PacketBreakConsoleScreen(Screen parent, PacketInstantBreakModule module) {
        super("发包秒破控制台", parent);
        this.module = module;
    ...
    private void buildInto(CompactStack stack) {
        ...
        switch (tab) {
            case OVERVIEW -> new PacketBreakOverviewPage(this, module).build(stack);
            case TARGET -> new PacketBreakSettingsPage(this, module).buildTarget(stack);
            case PACKET -> new PacketBreakSettingsPage(this, module).buildPacket(stack);
            case ANTICHEAT -> new PacketBreakSettingsPage(this, module).buildAnticheat(stack);
            case RENDER -> new PacketBreakSettingsPage(this, module).buildRender(stack);
        }
    ...
    private static final class Status {
        ...
        /** 取值全部来自模块设置、模块只读接口与协调器只读接口，本类不改任何状态 */
        private static Status capture(PacketInstantBreakModule module) {
            if (module == null) return empty();

            return new Status(new String[]{
                "§7模块：" + (module.isEnabled() ? "§a运行中" : "§8未启用"),
                "§7目标模式：§f" + module.settings().targetMode.displayName,
                "§7破坏方式：§f" + module.settings().breakMode.displayName,
                "§7队列：§f" + module.queueSize() + " §7块 §8/ §7已破坏 §f" + module.successCount() + " §7块",
                "§7当前目标：" + activeText(module),
                "§7服务器：" + (TacticalCoordinator.isServerLagging() ? "§c卡顿" : "§a流畅")
                    + " §8/ §7TPS §f" + String.format(Locale.ROOT, "%.1f", TacticalCoordinator.getCurrentTps())
            });
        }

        /** 当前目标与进度：无目标压成灰字，有目标显示坐标与百分比 */
        private static String activeText(PacketInstantBreakModule module) {
