...
/**
 * 种子挖矿正式模块 · <b>矿物的写入路径</b>（正式化第八阶段 236）。
 *
 * <p><b>它是什么</b>：一个矿物在本维度里<b>可能有几条完全不同的代码路径把方块写进世界</b>。
 * 这是从 26.1.2 源码里读出来的事实，不是分类学练习 —— 因为路径不同，
...
public enum SeedOreWritePath {
     ...
     * （{@code OreFeature.java:23-53} 的椭圆体算法，尺寸与空气暴露丢弃率由配置给出）。
     */
    ORE_FEATURE("矿物地物"),

    /**
     * 散落地物：{@code Feature.SCATTERED_ORE}（{@code ScatteredOreFeature.java:23-50}）。
     *
    ...
    /**
     * 化石处理器：{@code fossil_lower}（{@code CavePlacements.java:98-106}）+
     * {@code ProcessorLists#FOSSIL_DIAMONDS}（{@code ProcessorLists.java:607-611}）
     * 把煤矿规则替换成 {@code deepslate_diamond_ore}。
     *
     ...
     * {@code OreVeinifier}（{@code NoiseChunk.java:166-168}；类型表见 {@code OreVeinifier.java:63-65}）。
     *
     * <p><b>它发生在 NOISE 阶段而不是 FEATURES 阶段</b>，而且<b>下界整条关闭</b>
     * （{@code NoiseGeneratorSettings.java:99-113} 的 {@code nether(...)} 传 {@code false}）。
