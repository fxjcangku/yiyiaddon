...
/**
 * 种子挖矿正式模块 · <b>维度档案</b>（正式化第八阶段 236；26.2 语义移植）。
 *
 * <p><b>为什么必须有它</b>：把「下界」当成「主世界换个 Y 范围」是错的。离线 worldgen 上下文
 * （{@code OfflineWorldgenContext}）的四件东西里有<b>三件</b>按维度不同：</p>
 *
 * <ol>
 *     <li><b>生物群系预设</b>：{@code MultiNoiseBiomeSourceParameterLists} 的
 *         {@code NETHER} / {@code OVERWORLD} 两个各自独立的预设，下界只有 5 个生物群系；</li>
 *     <li><b>噪声设置</b>：{@code NoiseGeneratorSettings} 的 {@code OVERWORLD} / {@code NETHER}，
 ...
 *         （{@code NoiseGeneratorSettings.nether(...)} 实参 vs {@code overworld(...)}），
 *         而 {@code RandomState.create(...)} <b>就是按这个布尔选随机算法的</b>。</li>
 * </ol>
 *
...
public enum SeedDimensionProfile {
    ...
    /**
     * 主世界：{@code DimensionTypes} 的 overworld 类型（min_y = -64，height = 384）、
     * 预设 {@code MultiNoiseBiomeSourceParameterLists.OVERWORLD}、
     * 噪声设置 {@code NoiseGeneratorSettings.OVERWORLD}（矿脉开、含水层开、非 legacy 随机）。
     */
    OVERWORLD("主世界", Level.OVERWORLD, MultiNoiseBiomeSourceParameterLists.OVERWORLD,
            NoiseGeneratorSettings.OVERWORLD, -64, 384),

    /**
     * 下界：{@code DimensionTypes} 的 the_nether 类型（min_y = 0，height = 256，logical_height = 128）、
     * 预设 {@code MultiNoiseBiomeSourceParameterLists.NETHER}、
     * 噪声设置 {@code NoiseGeneratorSettings.NETHER}（矿脉关、含水层关、<b>legacy 随机</b>）。
     */
