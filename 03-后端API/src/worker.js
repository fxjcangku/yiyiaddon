 ...
 * 5. 在线状态仅以心跳为准：/api/heartbeat 每 15 秒上报一次（写库节流为 30 秒），心跳超时 90 秒判定离线；断开或退出游戏时调用 /api/offline 立即离线
 */
...
// 心跳写库节流：客户端每 15 秒心跳一次，但同一玩家每 30 秒才真正写一次 users 表。
// 原因（2026-09-18 实机事故）：D1 免费版每天 10 万行写入，全量心跳写库时几个小时就被打满，
// 之后 /api/register 与 /api/heartbeat 全部 500（客户端首页「用户排名」显示 --、在线状态也停更）。
// 节流后写入量降到约 1/2，且在线/位置/延迟的显示粒度仍是 30 秒，够用。
// 注意节流只省 D1 行写，不省请求数（Workers 免费版按请求数算，2026-09-23 曾被打满返回 1027），
// 所以客户端心跳间隔同时从 3 秒放宽到 15 秒。
...
export default {
  async fetch(request, env) {
    ...
    // 心跳上报（公开，客户端每 15 秒调用一次，维持在线状态并上报延迟/模块/活动）
